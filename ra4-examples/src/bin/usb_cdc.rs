...
fn setup_pll_48mhz() {
    ...
    if system.pllcr().read().0 != 0 {
        ...
        system
            .pllcr()
            .write_value(pac::system::regs::Pllcr(0x00));
    }
    for _ in 0..100_000 {
...
fn main() -> ! {
    ...
    let usb_dev = UsbDeviceBuilder::new(usb_bus, UsbVidPid(0x1209, 0x4d31))
        ...
        .unwrap()
        .strings(&[
            StringDescriptors::new(usb_device::LangID::EN_US)
    ...
    loop {
        if let (Some(usb_dev), Some(serial)) = (
            unsafe { (&mut *USB_DEVICE.0.get()).as_mut() },
            unsafe { (&mut *USB_SERIAL.0.get()).as_mut() },
        ) {
